...
void* find_prime_max(void *arg)
{
     struct thread_param *param = (struct thread_param *) arg;
     int ctr = param->thread_ctr;

     if(is_prime(param->array[ctr])){
        param->prime_max = function(param->array[ctr]);
    }
     else
        param->prime_max = -1;
...
int main(int argc, char **argv)
{
  ...
  for(ctr=0; ctr < num_threads; ++ctr){
        ...
        if(pthread_create(&param->tid, NULL, find_prime_max, param) != 0){
  ...
  for(ctr=0; ctr < num_threads; ++ctr){
        ...
        if(ctr == 0 || (ctr > 0 && param->prime_max > prime_max)){
             prime_max = param->prime_max;    
             max_index = param->max_index;
        }
  }

  if(prime_max==-1)
      USAGE_EXIT("no prime in random generated numbers");
     
  printf("Max = %d for %d at index=%d\n", prime_max, a[max_index], max_index);
