...
void* operate(void* arg)
{
    ...
    while(ctr < param->size){
        ...
        if(arr1[ac1-1001].is_serial == 1 || arr1[ac2-1001].is_serial==1){
            ctr += param->skip;
            continue;
        }
...
int main(int argc, char** argv){
    ...
    Operation arr2[transactions];
    char line2[1024];
    
    for(int i = 0; i < transactions && fgets(line2, sizeof(line2), stream2) != 0 ; i++) {
    ...
    {
        if(arr2[i].type == 3){
            arr1[arr2[i].number1-1001].is_serial = 1;
        }
    }
    ...
    for(int i = 0; i < transactions ; i++)
    {
        if(arr1[arr2[i].number1 - 1001].is_serial || arr1[arr2[i].number2 - 1001].is_serial){


            if(arr2[i].type == 1){
                arr1[arr2[i].number1-1001].balance += 99*(arr2[i].amount/100.0); 
            }

            else if(arr2[i].type == 2){
                arr1[arr2[i].number1-1001].balance -= 101*(arr2[i].amount/100.0);
            }

            else if( arr2[i].type == 3 ){

                arr1[arr2[i].number1-1001].balance += (71*(arr1[arr2[i].number1-1001].balance))/1000;
            }

            else{
                arr1[arr2[i].number1 - 1001].balance -= 101*(arr2[i].amount/100.0);
                arr1[arr2[i].number2 - 1001].balance += 99*(arr2[i].amount/100.0);
            }
        }
    }
